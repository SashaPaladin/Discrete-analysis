...
private:
    ...
    static void RotateLeft(TNode *&localRoot);

    static void RotateRight(TNode *&localRoot);
...
bool Treap::InsertNode(const std::string &key, const unsigned long long &value) {
    ...
    if (key < root->key) {
        if (!InsertNode(root->left, key, value)) {
            return false;
        }
    ...
    if (!InsertNode(root->right, key, value)) {
        return false;
    }
...
bool Treap::InsertNode(TNode *&localRoot, const std::string &key, const unsigned long long &value) {
    ...
    if (key < localRoot->key) {
        if (!InsertNode(localRoot->left, key, value)) {
            return false;
        }
    ...
    if (!InsertNode(localRoot->right, key, value)) {
        return false;
    }
